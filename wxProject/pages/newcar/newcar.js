...
Page({
  data: {
    ...
    car_v_type: 2,         //1:二手车 2:新车 3:平行进口车
  ...
  getCarsBrandList:function(){
    var _this = this;
    wx.request({
      url: this.data.serviceUrl + '/v1/home/brand?token=' + app.globalData.token + '&device_code=' + _this.data.deviceType + '&page=1&rows=2&type=1',
      method: 'POST',
      data: {
        version: '33',
        status:1,
  ...
  jumpMtScroll(e) {
    ...
    if (rY >= 0) {
      ...
      if ( this.data.jumpNum != ('index' + (index + 1)) && 0 <= index < list.length) {
  ...
  chooseRules:function(e){
    ...
    if (this.data.car_second_type == ruleid && this.data.car_first_type == firstid) {
      this.setData({
        car_second_type: '',
        car_first_type:'',
      ...
      this.setData({
        car_second_type: ruleid,
        car_first_type: firstid
      })
    }
  },
  //输入价格
  ...
  getCarsListLoad: function () {
    ...
    wx.request({
      ...
      data: {
        ...
        natur_use: _this.data.car_natur_use,   //车是否运营
      ...
      success: function (res) {
        ...
        if (res.data.code == 1) {
          ...
          for (var li = 0; li < res.data.data.list.length; li++){
            var timeS = res.data.data.list[li].create_time;
            var month = new Date(timeS * 1000).getMonth() + 1;
            if (month < 10) {
              month = '0' + String(month)
            }
            var timeObj = new Date(timeS * 1000).getFullYear() + '-' + month
            // var imgObj = res.data.data.list[li].img.replace(/http/, "https")
            res.data.data.list[li].create_time = timeObj
            // res.data.data.list[li].img = imgObj
